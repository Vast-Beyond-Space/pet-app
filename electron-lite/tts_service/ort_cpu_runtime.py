...
class OrtCpuRuntime:
    ...
    def encode_audio_to_codes(self, audio_path: str | Path) -> list[list[int]]:
        """
        将参考音频 WAV 文件编码为 audio codes，用于音色克隆。

        Args:
            audio_path: WAV 文件路径（建议 16kHz 单声道 16bit PCM）

        Returns:
            list[list[int]]: audio codes，每帧各 VQ 通道的 token ID 列表
        """
        ...
        # 3. 重采样到 codec 目标采样率
        target_sr = int(self.codec_meta.get("codec_config", {}).get("sample_rate", sr))
        if sr != target_sr:
            # 简单线性插值重采样
            ratio = target_sr / sr
            num_samples = int(len(audio) * ratio)
            resampled = np.zeros(num_samples, dtype=np.float32)
            for i in range(num_samples):
                src_idx = i / ratio
                low = int(src_idx)
                high = min(low + 1, len(audio) - 1)
                frac = src_idx - low
                resampled[i] = audio[low] * (1.0 - frac) + audio[high] * frac
            audio = resampled
        ...
        # 5. 构造输入张量 [1, 1, samples]
        audio_tensor = audio[np.newaxis, np.newaxis, :].astype(np.float32)

        # 6. 运行 codec_encode
        encode_session = self.sessions["codec_encode"]
        input_objs = encode_session.get_inputs()
        output_names = [out.name for out in encode_session.get_outputs()]

        feeds: dict[str, np.ndarray] = {}
        for inp in input_objs:
            name = inp.name
            lower = name.lower()
            if "audio" in lower and "length" not in lower:
                feeds[name] = audio_tensor
            elif "length" in lower:
                feeds[name] = np.asarray([audio_tensor.shape[-1]], dtype=np.int32)
            else:
                shape = tuple(dim if dim > 0 else 1 for dim in inp.shape)
                feeds[name] = np.zeros(shape, dtype=np.int32)

        outputs = encode_session.run(None, feeds)
        named_outputs = dict(zip(output_names, outputs, strict=True))
        ...
        for name, value in named_outputs.items():
            lower = name.lower()
            if "code" in lower and "length" not in lower:
                audio_codes = value
                break

        if audio_codes is None:
            raise RuntimeError(f"codec_encode 输出中未找到 audio_codes。可用输出: {list(named_outputs.keys())}")

        # 8. 转为 list[list[int]]
        audio_codes = np.asarray(audio_codes, dtype=np.int32)
        if audio_codes.ndim == 3:
            frames = audio_codes[0].tolist()  # [frames, num_quantizers]
        elif audio_codes.ndim == 2:
            frames = audio_codes.tolist()
        else:
            frames = audio_codes.reshape(-1, 1).tolist()

        logger.info(f"Encoded {audio_path} -> {len(frames)} frames, {len(frames[0]) if frames else 0} channels")
        return frames
